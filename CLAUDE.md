...
`data/staging/*.csv` are committed (real extracted snapshots, not
placeholders); `data/normalized/*.csv` are gitignored build output
(regenerate with `make dbt-build`). `releases/` stays empty until a real
release is published — `python -m pipelines.cli release --version X.Y.Z`
populates it, gated on `reports/validation/validation_report.json` reporting
