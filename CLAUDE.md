...
- `champions-business-case.md` — why this exists: problem statement, value
  drivers, risks, recommendation
- `PRD.md` — product requirements: goals, scope/non-goals, functional and
  non-functional requirements, milestones (M1–M6)
- `V1-DATASET-SPEC.md` — the authoritative spec for the v1 dataset: entity
...
- `DATASET.md` — catalog of external data sources with extraction notes,
  including which are in v1 scope vs. deferred to later phases

When these documents disagree on scope or details, `V1-DATASET-SPEC.md` is
the most current source of truth for the v1 build; `PRD.md` is the source of
truth for product-level goals/non-goals.
...
- **Deferred sources**: PokéBase app, Limitless VGC, Victory Road (see
  `DATASET.md` and `V1-DATASET-SPEC.md` for why)
...
- **Provenance is mandatory.** Per `V1-DATASET-SPEC.md`, every record in
...
- **Respect the entity/key contracts.** Use the primary keys and join keys
  defined in `V1-DATASET-SPEC.md` (e.g. `pokemon_key` as the cross-source
...
- **Don't ship ungated data.** Anything landing in `releases/` should have
  ...
  gates described in `V1-DATASET-SPEC.md`, with results recorded under
  `reports/validation/`.
...
- **Update docs alongside scope changes.** If v1 scope, schema, or refresh
  policy changes, update `V1-DATASET-SPEC.md` (and `PRD.md` if goals/scope
  change) rather than letting the docs drift from actual implementation.
