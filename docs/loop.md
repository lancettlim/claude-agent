...
Ships items already on the `todo.md` checklist.
...
Turns gaps and open questions into new, well-formed `todo.md` items instead
of leaving them implicit.

1. Scan `dataset-spec.md`, `prd.md`, `data-sources.md`, and any validation
   reports under `reports/validation/` for work that isn't yet represented
   as a `todo.md` checkbox (new entities, unaddressed risks, open questions,
   newly deferred-then-revived sources).
2. Write each gap as one action-sized `todo.md` item under the right phase
   heading, following the existing bullet style (imperative, references the
   concrete file/table/field it touches).
3. Remove or reword stale items that no longer match the current spec.
4. Commit the `todo.md` update alone — this loop only edits the backlog, it
   doesn't implement anything.

```
/loop 30m Groom docs/todo.md: find one gap between dataset-spec.md/prd.md
and the current checklist, add or correct one item for it, and commit.
```
...
1. Look for one concrete piece of debt: duplicated text between docs,
   ...
   wording drifted from `dataset-spec.md`.
...
1. Re-read `prd.md`'s open questions and non-goals, and `dataset-spec.md`'s
   deferred sources, for one candidate post-v1 goal (e.g. a deferred source,
   a milestone beyond M6, a non-goal worth revisiting).
...
3. Add it to a `## Future goals` section in `prd.md` (create the section if
   it doesn't exist yet) rather than inventing a new file — keep the doc
   count harmonized.
4. Do not create `todo.md` checklist items from this loop; that's a
   deliberate handoff to backlog grooming once a goal is accepted.
5. Commit the design note alone.

```
/loop 30m Draft one post-v1 future-goal design note in prd.md's
"Future goals" section, based on an open question, non-goal, or deferred
source. Don't add todo.md items. Commit.
```
