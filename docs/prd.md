...
- ~~Which dashboard tool stack and hosting model should be used for Phase 1?~~
  **Resolved**: a static HTML/CSS/vanilla-JS site (Jinja2-rendered, Chart.js
  loaded from a CDN, no backend, no build tooling), generated by
  `pipelines/dashboard/` and deployed via GitHub Pages serving `/docs` on
  the default branch (the dashboard itself lives at `docs/dashboard/`).
  Chosen for zero server/hosting cost and minimal new dependencies,
  consistent with this repo's existing lightweight Python/dbt tooling. A
  dynamic Python/Streamlit dashboard is a possible future upgrade once the
  dataset has enough snapshots/trend data to justify the added hosting
  complexity — see `docs/todo.md`'s M6 backlog item. See `docs/dashboard.md`
