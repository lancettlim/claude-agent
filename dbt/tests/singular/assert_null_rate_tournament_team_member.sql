...
{{ config(fail_calc='max(null_rate_bps)', error_if='>100', warn_if='>100', meta={'category': 'null_rate', 'table_name': 'tournament_team_member'}) }}
select
  case when count(*) = 0 then 0 else round((
    sum(case when team_member_id is null then 1 else 0 end) +
